...
class SAMNeuron():
    def __init__(self, labels, mode, pathe2sam = './synthetic-neurons-dataset/Grounded-Segment-Anything/'):
        ...
        self.predictor = SamPredictor(sam_model_registry[sam_version](checkpoint=sam_checkpoint).to(device))
        self.labels = labels
        self.mode = mode
    ...
    def show_mask(self, mask, image):
        image_data = (image.astype(np.float32)-image.min()) /(image.max()-image.min())
        h, w = mask.shape[-2:]
    ...
    def calc_activations(self, images): # calculate mask and activations for an input image
        ...
        for image_pil in images:
            ...
            for text_prompt in text_prompt_list:
                ...
                boxes_filt, pred_phrases = self.get_grounding_output(
                    self.model, image, text_prompt, box_threshold, text_threshold, device=device
                )
                ...
                if boxes_filt.shape[0] != 0:
                    ...
                    transformed_boxes = self.predictor.transform.apply_boxes_torch(boxes_filt, image.shape[:2]).to(device)
